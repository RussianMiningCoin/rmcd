...
We do not recommend Windows for rmcd production use at this time. Currently,
...
process and clone `git@github.com:CryptoManiac/rmcd.git`. Otherwise follow this
...
5. Select either the `rmcd.exe` (unity) or `rmcd_classic.exe` (non-unity)
   ...
   rmcd repo, e.g. `datagen.vcxproj`. Please ignore them.
...
```
.\build\x64-Release\Release\rmcd(_classic).exe
```
or
```
.\build\x64-Debug\Debug\rmcd(_classic).exe
```
...
(unity build) project or the `rmcd_classic` (non-unity) project, and
right-click to build.

The executable will be in 
```
.\build\cmake\Release\rmcd(_classic).exe
```
 or
````
.\build\cmake\Debug\rmcd(_classic).exe
````
where these paths are relative to your cloned git repository.
