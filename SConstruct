...
if (not platform.machine().endswith('64')):
    print('Warning: Detected {} architecture. RMC requires a 64-bit OS.'.format(
          platform.machine()));
...
def check_openssl():
    ...
    if parse_time(d) < parse_time(build_time):
        raise Exception('Your openSSL was built on %s; '
                        'rmcd needs a version built on or after %s.'
                        % (line, build_time))
...
for tu_style in ['classic', 'unity']:
    for toolchain in all_toolchains:
        for variant in variants:
            ...
            target = env.Program(
                target=os.path.join(variant_dir, 'rmcd'),
...
vcxproj = base.VSProject(
    os.path.join('Builds', 'VisualStudio2015', 'RMCD'),
    source = [],
    VSPROJECT_ROOT_DIRS = [
        'build/',
        'src/beast/extras',
